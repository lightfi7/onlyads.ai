...
export default function TopProducts() {
  ...
  }, [subPage, rowsPerPage, handleScroll]);

  useEffect(() => {
    setData_(data.stores.slice(0, (subPage + 1) * 2));
  }, [subPage, data.stores]);
  ...
  useEffect(() => {
    if (data.inited) dispatch(getStores(data.params));
    setSubPage(0);
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    dispatch(
      setParams({ ...data.params, page_size: event.target.value, page: 0 })
    );
    setPage(0);
  };
                  ...
                  return (
                    <Box key={p.id}>
                      ...
                      >
                        ...
                        >
                          <Avatar src={p.main_image} variant="rounded" />
                          <Box>
                            <Typography
                              variant="subtitle1"
                              noWrap
                      ...
                      <Divider>Top Best Selling Products</Divider>
                      <Box
                        sx={{
                          display: "flex",
                          justifyContent: "space-between",
                      ...
                      >
                        {Object.values(p.best_selling_products?.month).map(
                          (c) => {
                            return (
                              <Box
                                sx={{
